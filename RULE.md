...
- 【强制】Pi `QA_AGENT` 与 `CODING_AGENT` 使用 credential-relay 的 `--internal` 网络，agent 不得直连 npm/pypi。浏览器 QA 或隔离 Coding 且仓库含 `package.json` 时，宿主必须先用该角色镜像在 **bridge** 上安装依赖（native addon 按 Linux ABI），再启动隔离 agent；agent 侧 `npm_config_offline=true`。Coding 预安装写入任务 `repo/` 与任务级 `/work/cache`。QA 预安装写入 `provider-attempts/<stageRunId>/repo/` 与同一任务级 `/work/cache`，禁止把 QA 挂到任务根 `repo/`，也禁止把 QA 加入 `READ_ONLY_REPO_ROLES`（`npm run build` 需要写 `.next/`）。禁止给 Pi agent 放开公网来“修好” `EAI_AGAIN`/`ENOTCACHED`。Coding 在依赖仍缺失时不得把 DNS/安装失败报成交付失败：代码改动完成后 `status=SUCCESS` 且 `testStatus=SKIPPED`，安装/构建由后续 `HOST_VERIFY` 执行。docs-only QA 跳过安装。
  - npm 预安装硬超时至少 20min（`max(1_200_000, bashCommandTimeoutMillis)`），必须低于命令 deadline（默认 1h）。失败文案必须先报 `timed out`/`exit code`，禁止把 `npm notice` / `npm warn deprecated` 截成唯一诊断。预安装容器可继承宿主 `npm_config_registry` / `NPM_CONFIG_REGISTRY`（仅 bridge provision）；隔离 agent 不得继承 registry。
  - 代码：`QaNpmInstallPlan`、`DockerPiAgentExecutor.provisionQaNpmDependencies`、`DockerPiAgentExecutor.containerFailureMessage`、`RepairWorkspaceFactory.createProviderAttempt`、`ProcessContainerRunner` 的 `--entrypoint`、`RequirementAgentStageOrchestrator` Coding 角色指令、`deploy/cloud-server/start-backend.sh`
  - 验证：`./mvnw -pl exec -am -Dtest=QaNpmInstallPlanTest,DockerPiAgentExecutorTest#shouldProvisionNpmDependenciesOnBridgeBeforeIsolatedQaAgent,DockerPiAgentExecutorTest#shouldProvisionNpmDependenciesOnBridgeBeforeIsolatedCodingAgent,DockerPiAgentExecutorTest#shouldFailQaAsEnvironmentWhenNpmProvisionExitsNonZero,DockerPiAgentExecutorTest#shouldFailQaAsEnvironmentWhenNpmProvisionTimesOutWithoutHidingExitCodeBehindNpmNotices,DockerPiAgentExecutorTest#shouldRouteQaAgentSnapshotToTheQaImage,DockerPiAgentExecutorTest#shouldMountQaRepoFromAnIndependentProviderAttemptWorkspace,DockerPiAgentExecutorTest#shouldGiveQaAnIndependentCandidatePatchWorkspaceWhileRetainingTaskCache -Dsurefire.failIfNoSpecifiedTests=false test`
...
- 【强制】工作区中会被容器 rw 挂载的目录（`repo/`、`output/`、`cache/`）创建时必须放开 POSIX 权限使容器内非特权用户（Pi 为 1000:1000）可写。Linux bind mount 强制宿主侧权限，root 创建的 0755 目录会让 agent 以 EACCES 秒退且零事件（macOS virtiofs 不强制宿主权限，本地不会暴露此缺陷——2026-08-21 杭州迁移真机实测）。不得用"容器改跑 root"替代本约束。**且 git prepare（clone/fetch/checkout 以宿主 root 身份写入 repo 树）之后、容器启动之前必须对 repo 树递归放权**（`RepairWorkspaceFactory.makeContainerTreeWritable`：目录 rwxrwxrwx、文件补齐 group/other 读写并保留执行位）：仅放开顶层目录不够，clone 出的子目录仍是 root 0755，QA npm provision 写 node_modules、agent git commit 写 .git 都会 EACCES（2026-08-21 真机第二次命中同类问题）。
...
- 【强制】宿主 `git clone` / `git fetch origin <base>` / `git push` 对 GitHub TLS 闪断（`SSL_ERROR_SYSCALL`、`GnuTLS recv error`、`unable to access`、连接重置）最多重试 3 次。失败 clone 必须清空目标 `repo/`；失败 push 不得清空已提交的本地 commit。Push 前将仓库 `http.version` 设为 `HTTP/1.1`（Ubuntu GnuTLS + HTTP/2 会在 github.com 上 `-110` 断连），并设 `http.postBuffer=524288000`。生产宿主 `rd.executor.docker.git.timeout-seconds` 不得低于 600（默认 120 会把慢 pack 误杀成 `git command timed out`）。禁止设 `GIT_SSL_NO_VERIFY`。认证失败、401/403/404、仓库不存在不得当闪断重试。Push 过程中的 TLS 中断或超时在账本上视为 `UNKNOWN_REMOTE_RESULT`（可能已经写入远端），禁止盲重放；确认缺席后再 push。
  - 代码：`ProcessGitRepairWorkspaceRepository.runGitRetryingTransientNetwork`、`RequirementDeliveryEngine.isAmbiguousRemoteFailure`、`deploy/cloud-server/start-backend.sh`
  - 验证：`./mvnw -pl bootstrap,engine -am -Dtest=ProcessGitRepairWorkspaceRepositoryTest#shouldClassifyLibreSslGithubDropsAsTransient,ProcessGitRepairWorkspaceRepositoryTest#shouldRetryTransientTlsDropsOnWorkBranchPushWithoutClearingTheRepo,RequirementDeliveryStageProposalTest#publicationGnutlsPushFailureMarksUnknownAndCarriesPublicationReceipt -Dsurefire.failIfNoSpecifiedTests=false test`
...
- 【强制】需求任务写入 `COMPLETED` 的 finalize 事务必须同时写 `rd_task_completion_bindings`（`audit_run_id` / `state_version` / `state_hash`）。`RdTaskStatus.COMPLETED` 作为需求任务目标只允许出现在 `RequirementDeliveryEngine#planCompletionStage`、`#executeCompletionStage` 与 `RdTaskTransitionPolicy`。`RepairTaskMergeSyncEngine` 的 `COMPLETED → MERGED` 是读取源状态。由 `RequirementCompletionWriterPolicyTest` 扫描钉住；验证：
  `./mvnw -pl bootstrap -am -Dtest=RequirementCompletionWriterPolicyTest -Dsurefire.failIfNoSpecifiedTests=false test`。
- 【强制】`AuditedTaskState` 只能在 `RequirementStageFinalizationPort.recordOutcome` / `finalize` 边界内由冻结 plan 的 `auditedStateMutation` 写回；同 `state_version` 只允许同 hash 幂等重放。生产默认 `rd.requirement-delivery.audited-writeback.gate-mode=ENFORCE`。
- 【强制】`ROLE_EXECUTION:CODING_AGENT` 成功的 continuation 必须是 `HOST_VERIFY`；不得再直接续派 `ROLE_EXECUTION:QA_AGENT`。宿主验证回放命令的 `workBranch` 在 `rd_tasks.work_branch` 仍为空时必须回落为 `requirement/{taskId}`（与 `EngineRequirementExecutorAdapter` 一致），禁止把 `workBranch must not be blank` 抛成 DEAD_LETTER；prepare 失败必须落 `rd_host_verification_runs`（`ENVIRONMENT`）。
  Node 仓库 BUILD/STATIC 自动探测必须遍历与 `QaNpmInstallPlan` 相同的目录（`.` / `server` / `client` / `frontend` / `web` / `ui`）；根目录没有 `scripts.test`/`scripts.build` 时不得把 `install:all`/`dev`/`start` 当成 BUILD。嵌套包必须发 `npm --prefix <dir> …`，禁止 `cd <dir> &&`（`HostVerificationCommandRunner` 是 `ProcessBuilder` 无 `sh -c`）。只有仓库根目录可回落 `npx tsc --noEmit`；嵌套包仅在存在 `scripts.typecheck` / `scripts.lint` 时发 STATIC，禁止 `npm --prefix <dir> exec -- tsc --noEmit`（cwd 是仓库根，tsc 会打出帮助文本并以 exit 1 失败）。SKIPPED 的 BUILD/STATIC 仍必须写入 `VERIFY_BUILD_LOG`/`VERIFY_STATIC_LOG` 证据，否则 `SUCCEEDED` 无法过审计门。宿主 PATH 必须有 `node`/`npm`，否则下一步才是 `ENVIRONMENT`（exit 127）。
  操作员重试 `HOST_VERIFY` 的首个 durable command 必须是 `HOST_VERIFY` 本身，不得再开 Coding attempt；该 command 必须继承 checkpoint 的 `sourcePolicyRunId`（禁止空 `policy_run_id`，否则 HOST_VERIFY 终局 provenance 无法落库）。操作员重试 `PR_PUBLICATION` 的首个 durable command 必须是 `PUBLICATION:<operationId>` 本身，不得再开 Coding attempt；该 command 同样必须继承 checkpoint 的 `sourcePolicyRunId`（空 `policy_run_id` 时 finalize 无法写入 `failedStage=PUBLICATION:<operationId>`，`retry-preview` 会 409 `RETRY_POINT_AMBIGUOUS`）。下游尚未终态的 `QA_AGENT` attempt 必须预绑定到同一 checkpoint。`PRODUCT_DEFECT` 的 Coding 补救只走 `HOST_VERIFY_FIX`（attempt ≤ 3）。checkpoint-bound 的 `HOST_VERIFY` 必须先 `RECOVERING → EXECUTING` 再写结局（与角色 command 相同）。
