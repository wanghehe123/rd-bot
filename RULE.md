...
- 【强制】Pi `QA_AGENT` 与 `CODING_AGENT` 使用 credential-relay 的 `--internal` 网络，agent 不得直连 npm/pypi。浏览器 QA 或隔离 Coding 且仓库含 `package.json` 时，宿主必须先用该角色镜像在 **bridge** 上安装依赖（写入任务 `repo/` 与 `/work/cache`，native addon 按 Linux ABI），再启动隔离 agent；agent 侧 `npm_config_offline=true`。禁止给 Pi agent 放开公网来“修好” `EAI_AGAIN`/`ENOTCACHED`。Coding 在依赖仍缺失时不得把 DNS/安装失败报成交付失败：代码改动完成后 `status=SUCCESS` 且 `testStatus=SKIPPED`，安装/构建由后续 `HOST_VERIFY` 执行。docs-only QA 跳过安装。
