...
export function KnowledgeListPage() {
  ...
  const knowledgeBaseTotalPages = Math.max(1, pageData?.pages || 1);
      ...
      {pageData ? (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>
              {Math.max(1, pageData.current)} / {knowledgeBaseTotalPages}
            </span>
            <Button
              variant="outline"
              size="sm"
              onClick={() => setPageNo((prev) => Math.min(knowledgeBaseTotalPages, prev + 1))}
              disabled={Math.max(1, pageData.current) >= knowledgeBaseTotalPages}
            >
              下一页
            </Button>
          </div>
