...
const statusDotClass = (status?: string | null) => {
  ...
  if (normalized === "pending" || normalized === "unknown") return "bg-slate-400";
...
export function KnowledgeDocumentsPage() {
  ...
  const documentTotalPages = Math.max(1, pageData?.pages || 1);
      ...
      <Card>
        <CardHeader>
          <div className="flex w-full min-w-0 flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex w-full flex-wrap items-center justify-start gap-2 xl:w-auto xl:flex-1 xl:justify-end">
              <Input
                value={searchInput}
          ...
          {pageData ? (
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  {Math.max(1, pageData.current)} / {documentTotalPages}
                </span>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setCurrent((prev) => Math.min(documentTotalPages, prev + 1))}
                  disabled={Math.max(1, pageData.current) >= documentTotalPages}
                >
                  下一页
                </Button>
              </div>
...
const uploadSchema = z.object({
  knowledgeType: z.string().default("document"),
  chunkStrategy: z.string().min(1, "请选择分块策略"),
  chunkSize: z.string().min(1, "请输入块大小"),
  overlapSize: z.string().min(1, "请输入重叠大小")
}).superRefine((values, ctx) => {
  const requireNumber = (value: string, field: keyof typeof values, label: string) => {
    if (Number.isNaN(Number(value))) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: `${label}必须是数字` });
    }
  };
...
function UploadDialog({ open, onOpenChange, onSubmit }: UploadDialogProps) {
  ...
  // 切换分块策略时，用 API 返回的默认值填充 chunkSize/overlapSize
  useEffect(() => {
    ...
    if (defaults["chunkSize"] !== undefined) {
      form.setValue("chunkSize", String(defaults["chunkSize"]));
    }
    if (defaults["overlapSize"] !== undefined) {
      form.setValue("overlapSize", String(defaults["overlapSize"]));
    }
  ...
  const handleSubmit = async (values: UploadFormValues) => {
    ...
    setSaving(true);
    try {
      const chunkConfig = JSON.stringify({
        chunkSize: Number(values.chunkSize),
        overlapSize: Number(values.overlapSize)
      });
          ...
          <form className="min-h-0 flex flex-1 flex-col" onSubmit={form.handleSubmit(handleSubmit)}>
            ...
            <div className="space-y-3 rounded-lg border p-3">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs text-muted-foreground font-normal">分块策略</FormLabel>
