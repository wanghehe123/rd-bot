...
为 RD 需求交付提供可追溯、可聚合、可告警的指标与管理视图，使操作者能够判断系统是否健康、慢在哪里、消耗多少资源以及证据是否完整，同时不改变任务执行和恢复语义。
...
#### Scenario: 只用终态样本计算交付成功率
- **WHEN** 窗口中同时存在运行中、成功和失败任务
- **THEN** 交付成功率只以可判定的成功与失败终态为分母，运行中任务单独展示

#### Scenario: 失败类别缺失
- **WHEN** 一个失败没有结构化失败类别
- **THEN** 系统将其计入受控的 UNKNOWN 类别并保留任务下钻入口，而不是把错误消息作为指标标签
